...
def submit_plot():
    if request.method == "POST":
        ...
        plt.figure()
        top10 = data.nlargest(10, column1)  # Select top 10 based on column1
        ...
        if graph_type == "bar":
            ...
            style.use('ggplot')
            plt.figure(figsize=(15, 10),dpi=200,facecolor='y')
            ...
            graph=[column1].value_counts()
            graph.head(5).plot(kind='pie', autopct='%1.1f%%')
            plt.title(f'Top 10 Distribution of {column1}')
